\begin{frame}
	\begin{center}
		\Huge Einleitung
	\end{center}
\end{frame}

\begin{frame}
	\frametitle{Ideenfindung}

    \begin{figure}
        \centering
        \includegraphics[width=\textheight]{bilder/Bild_Brainwriting.png}
        \caption[Brainwriting]{Brainwriting}
        \label{fig:Brainwriting}
    \end{figure}



\end{frame}

\begin{frame}{Aufbau des Spiels}
	\begin{figure}
		\centering
		\includegraphics[width=.9\textwidth]{bilder/start.png}
		\caption[Startscreen]{Startszene unseres Spiels \glqq CZGame\grqq}
		\label{fig:Startscreen}
	\end{figure}
\end{frame}

\begin{frame}{Aufbau des Spiels}
	\begin{figure}
		\centering
		\includegraphics[width=.9\textwidth]{bilder/f1}
		\caption[Vorgeschichte]{Vorgeschichte des Spiels}
		\label{fig:Vorgeschichte}
	\end{figure}
\end{frame}

\begin{frame}{Aufbau des Spiels}
	\begin{figure}
		\centering
		\includegraphics[width=.9\textwidth]{bilder/f2}
		\caption[Fachschaften]{Auswahl seiner Fachschaft}
		\label{fig:Fachschaftauswahl}
	\end{figure}
\end{frame}

\begin{frame}{Aufbau des Spiels}
	\begin{figure}
		\centering
		\includegraphics[width=.9\textwidth]{bilder/anleitung}
		\caption[Tutorial]{Tutorial des Spiels}
		\label{fig:Tutorial}
	\end{figure}
\end{frame}

\begin{frame}{Aufbau des Spiels}
	\begin{figure}
		\centering
		\includegraphics[width=.9\textwidth]{bilder/f4}
		\caption[Foyer]{Start des \glqq CZGame\grqq \ im Foyer}
		\label{fig:Foyer}
	\end{figure}
\end{frame}

\begin{frame}{Aufbau des Spiels}
	\begin{figure}
		\centering
		\includegraphics[width=.9\textwidth]{bilder/Chemiegang}
		\caption[Chemiegang]{Beispiel eines Ganges (hier der Chemiegang)}
		\label{fig:Chemiegang}
	\end{figure}
\end{frame}

\begin{frame}{Aufbau des Spiels}
	\begin{figure}
		\centering
		\includegraphics[width=.9\textwidth]{bilder/inforaum}
		\caption[Informatikraum]{Beispiel eines Raumes (hier der Informatikraum)}
		\label{fig:Informatikraum}
	\end{figure}
\end{frame}

\begin{frame}{Aufbau des Spiels}
	\begin{figure}
		\centering
		\includegraphics[width=.9\textwidth]{bilder/inventar}
		\caption[Inventar]{Geöffnetes Inventar mit Items}
		\label{fig:Inventar}
	\end{figure}
\end{frame}

\begin{frame}{Aufbau des Spiels}
	\begin{figure}
		\centering
		\includegraphics[width=.9\textwidth]{bilder/kleinestreppenhaus}
		\caption[kleines Treppenhaus]{Beispiel eines kleines Treppenhauses}
		\label{fig:kleines Treppenhaus}
	\end{figure}
\end{frame}